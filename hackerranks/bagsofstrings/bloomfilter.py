...
class BloomFilter(object):
    ...
    """
    Class for Bloom filter, using murmur3 hash function. Modified slightly from
    https://www.geeksforgeeks.org/bloom-filters-introduction-and-python-implementation/
        - Replaced usage of bitarray as to not require C compiler
        - Modified/fixed comments for both personal and reviewer understanding
    ...
    def get_size(n, p):
        """
        Calculate size of the bit array (m) to be used using following formula:
        m = -(n * lg(p)) / (lg(2)^2)
    ...
    def get_hash_count(m, n):
        """
        Calculate number of hash functions (k) to be used using following
        formula:
        k = (m/n) * lg(2)
